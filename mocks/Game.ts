...
function makeGameObject({ runtimeData, intents, memory, getUsedCpu, globals, sandboxedFunctionWrapper, getHeapStatistics, cpuHalt }) {
	...
	let gclLevel = Math.floor(Math.pow((runtimeData.user.gcl || 0) / C.GCL_MULTIPLY, 1 / C.GCL_POW)) + 1,
		gclBaseProgress = Math.pow(gclLevel - 1, C.GCL_POW) * C.GCL_MULTIPLY;

	let gplLevel = Math.floor(Math.pow((runtimeData.user.power || 0) / C.POWER_LEVEL_MULTIPLY, 1 / C.POWER_LEVEL_POW)),
	...
	for (let i in runtimeData.rooms) {
		register.byRoom[i] = {};
	...
	for (let i in runtimeData.rooms) {
		register.rooms[i] = new Room(i);
	}

	// for (let i in customObjectsInfo) {
	...
	for (let i in runtimeData.userPowerCreeps) {
		register.powerCreeps[i] = new PowerCreep(i as Id<PowerCreep>);
		game.powerCreeps[register.powerCreeps[i].name] = register.powerCreeps[i];
	}

	for (let i in runtimeData.roomObjects) {
		let object = runtimeData.roomObjects[i];
		...
		if (object.type == 'creep') {
			register._objects[i] = new Creep(i as Id<Creep>);
		...
		if (object.type == 'powerCreep') {
			if (register.powerCreeps[i]) {
				...
				register._objects[i] = new PowerCreep(i as Id<PowerCreep>);
			}
		...
		if (structureTypes[object.type]) {
			...
			if (register._objects[i] instanceof OwnedStructure) {
				if (runtimeData.userObjects[i]) {
		...
		if (object.type == 'source') {
			register._objects[i] = new Source(i as Id<Source>);
		...
		if (object.type == 'mineral') {
			register._objects[i] = new Mineral(i as Id<Mineral>);
		...
		if (object.type == 'deposit') {
			register._objects[i] = new Deposit(i as Id<Deposit>);
		...
		if (object.type == 'energy') {
			register._objects[i] = new Energy(i as Id<Energy>);
		...
		if (object.type == 'nuke') {
			register._objects[i] = new Nuke(i as Id<Nuke>);
		...
		if (object.type == 'tombstone') {
			register._objects[i] = new Tombstone(i as Id<Tombstone>);
		...
		if (object.type == 'ruin') {
			register._objects[i] = new Ruin(i as Id<Ruin>);
		...
		if (object.type == 'constructionSite') {
			register._objects[i] = new ConstructionSite(i as Id<ConstructionSite>);
			if (runtimeData.userObjects[i]) {
	...
	runtimeData.flags.forEach(flagRoomData => {
		...
		let data = flagRoomData.data.split("|");
		data.forEach(flagData => {
			if (!flagData) {
				return;
			}
			let info = flagData.split("~");
			info[0] = info[0].replace(/\$VLINE\$/g, "|").replace(/\$TILDE\$/g, "~");
			let id = 'flag_' + info[0];
			let flag = register._objects[id] = new Flag(info[0], info[1], info[2], flagRoomData.room, info[3], info[4]);
	...
	game.map = register.map = new Map(runtimeData, register, globals);

	game.market = register.market = new Market(runtimeData, intents, register) as any;
...
(function () {
	...
	exports.init = function (
		...
		let userId = _runtimeData.user._id;
	...
	exports.run = function (userId) {
		...
		let mainExports = runCodeCache[userId].globals.require('main');
